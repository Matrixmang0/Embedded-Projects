...
void LED_glow(unsigned long);
unsigned char arr[2];
unsigned long displayVal;
...
void main() {
    ...
    while(1) {
        ...
        ADCON0 = 0x95;
        while (ADCON0&0x04);
        unsigned int two_MSB = ADRESH;
        unsigned int eight_LSB = ADRESL;
        unsigned long AnalogVal = (two_MSB << 8) | eight_LSB;

        displayVal = ((AnalogVal*100)/1023);
        LCD_print(displayVal);
        LED_glow(displayVal);
    }
}
...
void display_init(void) {
    ...
    command(0x30);
    _delay((unsigned long)((5)*(6000000/4000.0)));
    command(0x30);
